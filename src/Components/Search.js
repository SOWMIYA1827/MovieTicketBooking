  ...
  const navigate = useNavigate();
    ...
    return(
        <div className="search">
            <h1 className="search-heading">Movie Ticket Booking</h1>
            <div className="search-list">
                <input type="text" placeholder="Search for movies..." className="search-input" onChange={(e) => setSearch(e.target.value)} />
           ...
           <div className="search-results">
                {movies.filter((movie) => movie.title.toLowerCase().includes(search.toLowerCase())).map((movie) => (
                    <div className="movie-card" key={movie.id}>
                        ...
                        <button onClick={() => bookMovie(movie)}>Book Now</button>
                    </div>
                ))}
            </div>
        </div>
